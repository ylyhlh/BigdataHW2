...
function hw2.FistaL1(D, params)
   ...
   local Lrate = params.Lrate or 0.5
   params.maxiter = params.maxiter or 50
   params.maxline = params.maxline or 10
   ...
   fista.pl = function (x,L)
		 fista.shrinkage(x,fista.lambda/L)
	      end
   fista.shrinkage = function(x, alpha)
     		 local z = torch.abs(x):add(-alpha)
		 z[torch.lt(x,0)] = 0
		 local sign = torch.abs(x)
		 sign[torch.gt(x,0)] = 1
	 	 sign[torch.lt(x,0)] = -1
	         x:copy( torch.cmul(z,sign) )
              end   

   fista.run = function(x, lam, codeinit)
		  lam = lam or params.lam
		  ...
		  return hw2.FistaLS(fista.f, fista.g, fista.pl, fista.code, params)
	       end
